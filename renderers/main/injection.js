...
window.sendUiNotifyMessage = (message, timeout = 3000, type = 'success') => {
  // Declara el aviso de restauración de conexión.
  const rest = document.getElementById('rest');
  ...
  if (type === 'success') {
      ...
      rest.innerHTML = `<p>${ message }</p>`;

      // Añade la clase visible.
      rest.classList.add('visible');

      // Elimina la clase visible pasados los 5 segundos.
      setTimeout(() => { rest.classList.remove('visible') }, timeout);
  } else {
      // Elimina la clase visible.
      rest.classList.remove('visible');

      lost.innerHTML = `<p>${ message }</p>`;

      // Añade la clase visible.
      lost.classList.add('visible');
      
      // Elimina la clase visible pasados los 5 segundos.
      setTimeout(() => { lost.classList.remove('visible') }, timeout);
  }
}
